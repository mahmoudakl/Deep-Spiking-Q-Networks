# this file contains inplementations of a SQN and a conversion method (according to Rueckauer et al.
# for source see thesis). The SQN class is adapted from SpyTorch (source, see thesis)
...
class SQN(nn.Module):
    ...
    def __init__(self, network_shape, device, alpha, beta, weight_scale=1, encoding='constant',
                 decoding='potential', threshold=1, simulation_time=100, reset='subtraction',
        ...
        """
        Args:
            network_shape: shape of the network given in the form [5,17,17,2] for 5 input neurons,
                           two hidden layers with 17 neurons and 2 output neurons
            ...
            two_input_neurons: for input methods 'equidistant' and 'poisson' if one output neuron is
                               to be used for negative and positive inputs each
            add_bias_as_observation: this option is for training using SpyTorch as SpyTorch allows
                                     no hidden layer biases. If true a 1 is added as constant input
        """
        self.alpha = alpha
        self.beta = beta
        ...
        for i in range(0, len(network_shape) - 1):
            ...
            torch.nn.init.normal_(self.weights[i], mean=0.0,
                                  std=weight_scale/np.sqrt(network_shape[i]))
    ...
    def forward(self, input_data):
        ...
        # if two input neurons is True, double the input size, where the first set of values
        # represents the positive inputs and the second set the negative inputs.
        if self.two_input_neurons:
            input_data = torch.cat([input_data * (input_data > 0),
        ...
        for l in range(0, len(self.weights)):
            syn.append(torch.zeros((batch_size, self.weights[l].shape[1]), device=self.device,
                                   dtype=torch.float))
            mem.append(torch.zeros((batch_size, self.weights[l].shape[1]), device=self.device,
                                   dtype=torch.float))

        # Here we define two lists which we use to record the membrane potentials and output spikes
        mem_rec = []
        spk_rec = []
        # Additionally we define a list that counts the spikes in the output layer, if the output
        # method 'spikes' is used
        if self.decoding == 'spikes':
            spk_count = torch.zeros((batch_size,self.weights[-1].shape[1]), device=self.device,
                                    dtype=torch.long)
        ...
        for t in range(self.simulation_time):
            ...
            if self.encoding == 'constant':
                ...
                spike_snapshot = torch.tensor(np.random.uniform(low=0, high=1,
                ...
                input = (torch.ones_like(input_data)*t == torch.round(
            ...
            for l in range(len(self.weights)):
                ...
                if l == 0:
                    ...
                    h = torch.einsum("ab,bc->ac", [spk_rec[len(spk_rec) - 1][l - 1],
                                                   self.weights[l]])
                # add bias
                if self.bias[l] is not None:
                    h += self.bias[l]

                # calculate the spikes for all layers (decoding='spikes' or 'first_spike') or for
                # all but the last layer (decoding='potential')
                if self.decoding == 'spikes' or self.decoding == 'first_spike'\
                        or l < len(self.weights)-1:
    ...
    def load_state_dict(self,layers):
        ...
        # if two input neurons are used, split the weights into two groups, one for positive and one
        # for negative inputs. The weights for the negative inputs are multiplied by minus 1
        if self.two_input_neurons:
            self.weights[0] = torch.cat([self.weights[0],torch.neg(self.weights[0])])

    def load(self, weights, biases, transpose=True):
        """
        Analog to previous method, but weights and biases are passed directly it is used for
        converting networks. For some reason, when converting, weights are saved in transposed form,
        so we need to transpose them back to the correct shape. Whether to transpose or not can be
        controlled with the parameter transpose
        """
        for l in range(0,len(weights)):
...
def weight_conversion(model, weights, bias, device, normalization_method='robust', ppercentile=0.99,
                      path_to_replay=None):
    """
    Args:
        model: the model that is converted
        weights: a list, where the first element is the weights of the first layer of the model and
                 so on
        bias: as weights, but for the bias
        device: device which torch is using
        normalization_method: 'robust' or 'model' or 'data' for the method from Rueckauer and the
                              two from Diehl respectively
        ppercentile: needs to be set to the desired percentile, for 'data' it is automatically set
                       to 1
    ...
    if normalization_method == 'model':
        ...
        for l in range(len(converted_weights)):
            ...
            # get the maximum weight in the layer, in case all weights are negative, max_pos_input
            # would be zero, so we use the max weight to rescale instead
    ...
    if normalization_method == 'robust' or normalization_method == 'data':
        ...
        if path_to_replay is None:
            raise Exception("Data based training method chosen, but no path to replay dataset "
                            "provided")
