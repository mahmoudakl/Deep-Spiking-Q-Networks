...
def optimize_model(memory, BATCH_SIZE, device, policy_net, Transition, n_actions, target_net, gamma,
                   optimizer, double_q_learning=True, gradient_clipping=True,
    ...
    if batch.state[0].dim() == 1:
        non_final_next_states = torch.stack([s.float() for s in batch.next_state if s is not None])
        state_batch = torch.stack([s.float() for s in batch.state])
    # use this if the input state has several dimensions
    else:
        non_final_next_states = torch.cat([s.float() for s in batch.next_state if s is not None])
    ...
    if not double_q_learning:
        ...
        # Double Q Learning
        next_state_values = torch.zeros(BATCH_SIZE, device=device)
        # We use Double Q Learning, that is the best action is chosen with the policy net, but it's
        # value is calculated with the target net
        ...
        best_action_mask[torch.arange(len(next_state_best_actions)),
                         next_state_best_actions.argmax(1)] = 1
...
def train_agent(environment, policy_net, target_net, batch_size, gamma, eps_start, eps_end,
    ...
    Args:
        ...
        target_net: the target network for the agent which should be a copy of the policy network
                    initially
        ...
        input_preprocessing: function that is used to preprocess the inputs, if None, no
                             pre-processing happens
                             This function needs to take the reward and the observation as input.
        reward_preprocessing: function that is used to preprocess the rewards, if None, no
                              pre-processing happens. This function needs to take only the
                              observation history as input
        ...
        torch_seed: manual seed for torch that was used to save in the hyperparameters file
                    (only for saving, setting this variable does not change the training process of
                    the agent)
        random_seed: manual seed for random that was used to save in the hyperparameters file
                    (only for saving, setting this variable does not change the training process of
                    the agent)
        update_frequency: (used for BreakOut) determines how many steps the agent does, before the
                          networks are updated
        no_op_range: (used for BreakOut) tuple: number of minimum and maximum "do-nothing"
                     operations at the beginning of each episode. If None the agent directly chooses
                     the first action
        no_op: (used for BreakOut) describes which operation is the "do-nothing" operation (int)
        observation_history_length: (used for BreakOut) keep a history of the last observations in a
                                    list. If None, no history is kept
    ...
    if gym_seed is not None:
        env.seed(gym_seed)
        # this line is necessary to reproduce our results, because it was included in the original
    ...
    for i_episode in range(1, num_episodes + 1):
        ...
        for t in count():
            ...
            # reward is added before preprocessing in order to plot the actual reward from the gym
            # environment
            total_reward += reward

            # pre-process the reward if specified
            if reward_preprocessing is not None:
                reward = reward_preprocessing(reward,observation)
            ...
            # Perform one step of the optimization (on the policy network) ever update_frequency
            # steps (default 1)
            if steps_done%update_frequency == 0:
                opt = optimize_model(memory, batch_size, device, policy_net, Transition, n_actions,
                                     target_net, gamma, optimizer,
